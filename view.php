...
if ($action === 'heartbeat') {
    ...
    try {
        if (!$DB->get_manager()->table_exists('ticket_system_presence')) {
            throw new Exception('Table missing');
        }

        // 1. Get Ticket Owner ID to identify the student.
        $ticketownerid = $DB->get_field('ticket_system_tickets', 'userid', ['id' => $id]);

        $presence = new stdClass();
        $presence->ticketid = $id;
        $presence->userid = $USER->id;
        $presence->timemodified = time();

        if ($existing = $DB->get_record('ticket_system_presence', ['ticketid' => $id, 'userid' => $USER->id])) {
            $presence->id = $existing->id;
            $DB->update_record('ticket_system_presence', $presence);
        } else {
            $DB->insert_record('ticket_system_presence', $presence);
        }

        $DB->delete_records_select('ticket_system_presence', 'timemodified < ?', [time() - 60]);

        $sql = "SELECT u.*
                FROM {ticket_system_presence} p
                JOIN {user} u ON p.userid = u.id
                WHERE p.ticketid = ? AND p.timemodified > ?";

        $viewers = $DB->get_records_sql($sql, [$id, time() - 30]);
        ...
        foreach ($viewers as $viewer) {
            $activeusers[] = [
                ...
                'avatar' => $OUTPUT->user_picture($viewer, [
                    'size' => 35,
                    'link' => false,
                    'class' => 'rounded-full',
                ]),
                'is_owner' => ($viewer->id == $ticketownerid), // This is the key.
            ];
        }

        echo json_encode(['status' => 'success', 'viewers' => $activeusers]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && confirm_sesskey()) {
    if (isset($_POST['reply_message'])) {
        $message = required_param('reply_message', PARAM_RAW);
        $reply = (object)[
            'ticket_id' => $id,
            'userid' => $USER->id,
            'message' => $message,
        ...
        $replyid = $DB->insert_record('ticket_system_replies', $reply);
        ...
        $DB->set_field('ticket_system_tickets', 'status', $newstatus, ['id' => $id]);
    ...
    if ($action === 'update_status' && $canmanage) {
        ...
        $oldstatus = $ticketrecord->status;
        $DB->set_field('ticket_system_tickets', 'status', $status, ['id' => $id]);
    ...
    if ($action === 'assign_user' && $canmanage) {
        ...
        $DB->set_field('ticket_system_tickets', 'assigned_to', $assignedto, ['id' => $id]);
    ...
    if ($action === 'update_category' && $canmanage) {
        ...
        $category = $DB->get_record('ticket_categories', ['id' => $catid], 'id, title', MUST_EXIST);
        $DB->set_field('ticket_system_tickets', 'category_id', $catid, ['id' => $id]);
    ...
    if ($action === 'add_internal_comment' && $canmanage) {
        ...
        $DB->insert_record('ticket_system_comments', $note);
    ...
    if ($action === 'submit_feedback' && !$canmanage && $ticketrecord->userid == $USER->id) {
        ...
        if (!$DB->record_exists('ticket_system_feedback', ['ticketid' => $id])) {
            $feedback = (object)[
                'ticketid' => $id,
                'userid' => $USER->id,
                'rating' => $rating,
            ...
            $DB->insert_record('ticket_system_feedback', $feedback);
...
$repliesrecords = $DB->get_records('ticket_system_replies', ['ticket_id' => $id], 'created_at ASC');
$replies = [];
foreach ($repliesrecords as $r) {
    $ruser = $DB->get_record('user', ['id' => $r->userid], '*', MUST_EXIST);
...
if ($canmanage) {
    if ($DB->get_manager()->table_exists('ticket_system_logs')) {
        $logs = $DB->get_records('ticket_system_logs', ['ticketid' => $id], 'timemodified DESC');
        foreach ($logs as $l) {
            $luser = $DB->get_record('user', ['id' => $l->userid]);
            $ticketlog[] = [
                'message' => fullname($luser) . ": " . $l->actionname,
    ...
    $latestselsql = "SELECT * FROM {ticket_system_tickets}
                     WHERE userid = ? AND id != ?
    ...
    if ($DB->get_manager()->table_exists('ticket_system_comments')) {
        $notesrecords = $DB->get_records('ticket_system_comments', ['ticketid' => $id], 'created_at DESC');
        foreach ($notesrecords as $note) {
            $noteuser = $DB->get_record('user', ['id' => $note->userid]);
            $internalnotes[] = [
                'user_name' => fullname($noteuser),
...
$feedbackrecord = $DB->get_record('ticket_system_feedback', ['ticketid' => $id]);
